...
func TestValidateCondition_WafCases(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			cond:    mkCond(t, "action_token.score", "ge", ptr("0.0"), nil, ptr("web")),
			wantErr: false,
		},
		{
		...
		},
		{
			...
			cond:    mkCond(t, "action_token.score", "le", ptr("1.0"), nil, ptr("web")),
			wantErr: false,
		},
		{
		...
		},
		{
			...
			cond:        mkCond(t, "action_token.score", "ge", ptr("-0.01"), nil, ptr("web")),
			wantErr:     true,
		...
		},
		{
			...
			cond:        mkCond(t, "action_token.score", "le", ptr("1.5"), nil, ptr("web")),
			wantErr:     true,
			errContains: "out of range",
		},
		{
			name:        "score above range",
			cond:        mkCond(t, "action_token.score", "le", ptr("1.01"), nil, ptr("web")),
			wantErr:     true,
			errContains: "out of range",
		},
		{
...
func conditionValidationRejectionCases() []conditionValidationRejectionCase {
	return []conditionValidationRejectionCase{
		...
		{name: "waf score below range", err: `out of range`, securityRule: `{ name = "waf-score-below", action = "log", condition = { or = [{ and = [{ field = "action_token.score", field_key = "web", operator = "ge", value = "-0.01" }] }] } }`},
		{name: "waf score above range", err: `out of range`, securityRule: `{ name = "waf-score-above", action = "log", condition = { or = [{ and = [{ field = "action_token.score", field_key = "web", operator = "le", value = "1.01" }] }] } }`},
		{name: "waf action token score below range", err: `out of range`, securityRule: `{ name = "waf-action-token-score-below", action = "log", condition = { or = [{ and = [{ field = "action_token.score", field_key = "web", operator = "ge", value = "-0.01" }] }] } }`},
...
func testAccBehaviorConditionAllOperators(name, certId string) string {
	behaviors := []string{
		...
		`{ name = "b-not-in", condition = { or = [{ and = [{ field = "http.request.method", operator = "not_in", values = ["OPTIONS"] }] }] }, actions = { cache_behavior = "BYPASS" } }`,
